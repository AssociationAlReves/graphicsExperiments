...
internal sealed class ConeRenderer : IVoronoiRenderer
{
    ...
    private const float PointSize = 5f; // site dot diameter, in pixels
    ...
    public void Load(GL gl, uint instanceVbo, int siteCount, int fbWidth, int fbHeight)
    {
        ...
        // 2x2 NDC square is 2/sqrt(N); ~6x that safely covers a cell with margin.
        float coneRadius = Math.Clamp(6f * 2f / MathF.Sqrt(siteCount), 0.05f, 3.0f);

        BuildConeMesh(coneRadius);
        BuildPrograms();
        BuildBuffers();
        BuildFramebuffer();
    }
    ...
    /// distance comparison.
    /// </summary>
    private void BuildConeMesh(float coneRadius)
    {
        ...
        for (int s = 0; s < ConeSegments; s++)
        {
            ...
            verts.AddRange([MathF.Cos(a0) * coneRadius, MathF.Sin(a0) * coneRadius, height]);
            verts.AddRange([MathF.Cos(a1) * coneRadius, MathF.Sin(a1) * coneRadius, height]);
        }
    ...
    public void Render()
    {
        ...
        _gl.BindTexture(TextureTarget.Texture2D, _idTex);
        _gl.Uniform1(_gl.GetUniformLocation(_edgeProgram, "uId"), 0);
        _gl.BindVertexArray(_emptyVao);
        _gl.DrawArrays(PrimitiveType.Triangles, 0, 3);

        // Pass 3: white site dots on top.
        _gl.UseProgram(_pointProgram);
        _gl.Uniform1(_gl.GetUniformLocation(_pointProgram, "uPointSize"), PointSize);
        _gl.BindVertexArray(_pointVao);
